 ...
 * }
 */
export const getNodeTimezoneConfig = (node: ESPRMNode | undefined) => {
  ...
  const timeService: ESPRMService | undefined = node.nodeConfig.services.find(
    (service) => service.type === ESPServiceType.TIME
  );

  const timezoneParam: ESPRMServiceParam | undefined =
    timeService?.params?.find(
      (param) =>
        param.type === ESPServiceParamType.TIME.TIMEZONE &&
 ...
 */
export const setUserTimeZone = async (user: ESPRMUser | null) => {
  if (!user) return;
