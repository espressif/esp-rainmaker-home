...
import { isCrossClusterInvokeMarker } from "@sdk-adaptors/ESPRMMatterBase/utils/common";
...
function installCrossClusterPatchForParam(
  ...
  proto.setValue = async function (value: unknown): Promise<unknown> {
    ...
    if (isCrossClusterInvokeMarker(encoded)) {
      ...
      if (!result.success) {
        throw new Error(result.error ?? "Matter cross-cluster invoke failed");
      }
      self.value = value;
      return {
        status: "success",
...
export function installRmngCrossClusterInvokePatch(): void {
  if (crossClusterPatchInstalled) return;

  installCrossClusterPatchForParam(
    ESPRMNGMatterDeviceParam as unknown as { prototype: MatterParamProto },
    () =>
      ESPRMNGMatterBase.ESPRMNGMatterControlAdapter as CrossClusterControlAdapter | null,
    (self) => self._nodeRef?.deref()?.matterNodeId,
  );
