 ...
 * ({@link ESPRMNGUser.subscribe}), which uses the registered
 ...
 * `ESPRMNGNode` via `addTransport`/`removeTransport`. So this module
 ...
 * @param user - The authenticated raw `ESPRMNGUser` that owns the subscription.
...
export async function startRmngLocalDiscoverySubscription(
    ...
    user.subscribe(ESPRMNGEventType.localDiscovery, (data: unknown) => {
        // Feed the CDF store; the CDF property-change sync in transformToESPCDFNode
        // projects the resulting availableTransports onto the raw ESPRMNGNode.
        cdfCallback(data);
    });
    ...
    return () => {
        user.unsubscribe(ESPRMNGEventType.localDiscovery);
